...
# Date: 08/05/23
# v 1.0 
# Replaced FASTP to Trim Galore, select concordant paired reads instead of remove unmapped reads
# remove raw BAM
# v 1.1
# avoid lexicographical mistakes while removing samples
...
mkdir ${workdir}/${prefix}/fastq/trimgalore
...
ml fastqc/0.12.0 fastp/0.23.2 bwa/0.7.17 samtools/1.16.1 trimgalore
...
trim_galore \
    ...
    -j $thr \
    --basename \${name} \
    --output_dir ${prefix}/fastq/trimgalore/ \
    --paired \
    ${prefix}/fastq/temp/\${name}_R1.fq.gz \
    ${prefix}/fastq/temp/\${name}_R2.fq.gz
...
# alingment
bwa mem -t $thr $ref ${prefix}/fastq/trimgalore/\${name}_val_1.fq.gz ${prefix}/fastq/trimgalore/\${name}_val_2.fq.gz > ${prefix}/bam/tmp.\${name}_raw.sam 

# convert to BAM and sort
samtools view -@ $thr -b ${prefix}/bam/tmp.\${name}_raw.sam | samtools sort -@ $thr > ${prefix}/bam/raw/\${name}.bam
...
# filter unmaped reads and keep only paired concordant alingments

samtools view -@ $thr -Sh -f 0x2 -q $qual -b ${prefix}/bam/raw/\${name}.bam > ${prefix}/bam/tmp.\${name}_mapped.bam

# remove raw bam

rm ${prefix}/bam/raw/\${name}.bam
